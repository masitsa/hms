...
//connect to database
	...
	function save_tests_format2($result,$format){
		
		$sql = "UPDATE lab_visit_results SET lab_visit_results_result = '$result' WHERE lab_visit_result_format ='$format'";
		//echo $sql;
		$save = new Database();
		$save->insert($sql);
	}
...
class invoice{
	...
	function get_patient_2($strath_no){
		 //connect to database
        $connect = mysql_connect("localhost", "root","jacinta123")
                    or die("Unable to connect to MySQL".mysql_error());
